...
def _service(own_templates=None, total_matching=None, deleted=0):
    ...
    db.sessiontemplates.find = MagicMock(return_value=find_result)
    db.sessiontemplates.count_documents = AsyncMock(
        return_value=total_matching if total_matching is not None else len(own_templates))
    db.sessiontemplates.delete_many = AsyncMock(
...
class TestListSelfDescribing:
    ...
    async def test_reports_filter_and_totals(self):
        ...
        res = await svc.list_session_templates(str(USER), {"include_common": False})
    ...
    async def test_not_truncated_when_all_returned(self):
        ...
        res = await svc.list_session_templates(str(USER), {})
...
class TestDeleteTemplate:
    ...
    async def test_requires_a_selector(self):
        svc, db = _service()
        res = await svc.delete_session_template(str(USER), {})
        assert res["success"] is False
        db.sessiontemplates.delete_many.assert_not_called()

    @pytest.mark.asyncio
    async def test_preview_without_confirm(self):
        svc, db = _service([_tmpl("Endurance 2 (Jul 16)"), _tmpl("Endurance 2")])
        res = await svc.delete_session_template(
    ...
    async def test_confirmed_delete_scopes_to_own_non_common(self):
        ...
        query = db.sessiontemplates.delete_many.call_args.args[0]
    ...
    async def test_no_match_returns_gracefully(self):
        ...
        res = await svc.delete_session_template(str(USER), {"name": "Does Not Exist"})
    ...
    async def test_referenced_templates_skipped_others_deleted(self):
        ...
        db.calendarevents.count_documents = AsyncMock(
            side_effect=lambda q: 3 if q["sessionTemplateId"] == own[0]["_id"] else 0)
        res = await svc.delete_session_template(str(USER), {"keep_only": ["nothing kept"], "confirm": True})
