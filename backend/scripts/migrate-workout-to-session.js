 ...
 *
 * Ordering note (load-bearing): unique indexes on renamed fields are DROPPED
 * before the $rename phase and RECREATED after it. Renaming workoutId →
 * sessionTemplateId while the unique {userId, workoutId} index is live would
 * E11000 on the second doc per user (missing field indexes as null); creating
 * the new unique index before the rename would fail the same way in reverse.
 *
 * Every phase is idempotent: a crashed --execute can be re-run. The preflight
 * audit doc is written once ($setOnInsert) so --verify always compares against
 * the ORIGINAL pre-migration counts, even after a partial run + retry.
 *
 ...
 *     add-predefinedworkouts-validator, dedupe)
 *  - the connecting user has renameCollection privilege
 *  - backend + ai-coach are SUSPENDED (no writers) when running --execute
 *    (a booted post-rename backend auto-creates empty new-name collections;
 *     phase 1 drops such empty leftovers, but never a non-empty target)
 */
...
async function main() {
  ...
  // Never fall back to a default DB silently — a wrong-database run is worse
  // than an aborted one.
  const uriDb = new URL(uri).pathname.replace(/^\//, '') || null;
  const dbName = DB_OVERRIDE || uriDb;
  if (!dbName) {
    throw new Error('No database in MONGODB_URI path and no --db given — refusing to guess.');
  }
  ...
  const listNames = async () => new Set((await db.listCollections().toArray()).map(c => c.name));
  ...
  // ---------- EXECUTE ----------
  // Phase 0: audit snapshot (write-once — survives partial-run retries) + dump
  await db.collection('_migration_audit').updateOne(
    { migration: 'workout-to-session', phase: 'preflight' },
    { $setOnInsert: { migration: 'workout-to-session', phase: 'preflight', at: new Date(), preCounts: pre } },
    { upsert: true }
  );
  ...
  for (const c of AFFECTED.filter(n => pre[n] !== null)) {
    // spawnSync with an args array: no shell interpolation, and the error
    // path below never echoes the URI.
    const r = spawnSync('mongodump', [`--uri=${dumpUri}`, `--collection=${c}`, `--out=${dumpDir}`], { stdio: 'pipe' });
    if (r.status !== 0) {
      const stderr = redact(r.stderr ? r.stderr.toString() : '(no stderr)');
      throw new Error(`mongodump failed for ${c} — aborting before any change: ${stderr}`);
    }
  }
  console.log(`Dump written to ${dumpDir}`);

  // Phase 1: collection renames (skip-if-done; drop EMPTY leftover targets —
  // a booted post-rename backend auto-creates them via ensureIndexes)
  let existing = await listNames();
  for (const [oldName, newName] of RENAMES) {
    ...
    if (existing.has(newName)) {
      const n = await db.collection(newName).countDocuments();
      if (n === 0) {
        await db.collection(newName).drop();
        console.log(`dropped empty leftover ${newName} (auto-created by a booted service)`);
      } else {
        throw new Error(`Both ${oldName} and ${newName} exist and ${newName} has ${n} docs — refusing to rename. Resolve manually.`);
      }
    }
  ...
  existing = await listNames();
  if (existing.has('workouts')) {
    ...
    for (const w of legacy) {
      ...
      const doc = {
        ...
        // Inserted with the OLD field name ('type'); phase 3b renames it to
        // 'discipline' along with every other sessionlogs doc.
        type: w.type || 'strength',
        startedAt,
        // Only a completed legacy workout gets a completedAt; planned/skipped
        // ones keep their status in _legacyStatus for auditability.
        completedAt: status === 'completed'
          ? (durationMin ? new Date(new Date(startedAt).getTime() + durationMin * 60000) : startedAt)
          : undefined,
      ...
      if (w.planId) doc.planId = w.planId;
      await db.collection('sessionlogs').insertOne(doc);
    }
    // Drop gate is a REAL DB-side reconciliation, not a loop counter.
    const legacyIds = legacy.map(w => w._id);
    const convertedCount = legacyIds.length === 0 ? 0
      : await db.collection('sessionlogs').countDocuments({ _id: { $in: legacyIds } });
    if (convertedCount === legacy.length) {
      await db.collection('workouts').drop();
      console.log(`converted ${convertedCount}/${legacy.length} legacy workouts docs → sessionlogs; dropped workouts`);
    } else {
      throw new Error(`workouts conversion incomplete (${convertedCount}/${legacy.length} present in sessionlogs) — NOT dropping`);
    }
  }

  // Phase 2b: scheduled_workouts disposition
  if (existing.has('scheduled_workouts')) {
  ...
  // Phase 3a: DROP indexes on fields about to be renamed — BEFORE the renames.
  // The unique {userId, workoutId} index would E11000 mid-updateMany once a
  // second doc per user has its workoutId removed (missing indexes as null).
  existing = await listNames();
  if (existing.has('usersessionmodifications')) {
    const usm = db.collection('usersessionmodifications');
    for (const idx of await usm.indexes()) {
      if (idx.key && idx.key.workoutId !== undefined) {
        await usm.dropIndex(idx.name);
        console.log(`dropped index ${idx.name} on usersessionmodifications`);
      }
    }
  }
  if (existing.has('sessiontemplates')) {
    const st = db.collection('sessiontemplates');
    for (const idx of await st.indexes()) {
      if (idx.name === 'workout_text_search') {
        await st.dropIndex('workout_text_search');
        console.log('dropped workout_text_search');
      }
    }
  }

  // Phase 3b: field renames + enum flips
  ...
  // usersessionmodifications — workoutId → sessionTemplateId (indexes already dropped in 3a)
  ...
  // plans — nested arrays need an aggregation-pipeline update ($rename can't
  // reach them). Per-week $cond guard: weeks without a workouts key pass
  // through untouched (never clobber an existing sessions array with []).
  ...
  // Phase 3c: CREATE the new indexes — only after the renames put real values
  // in the new fields (a unique index over all-missing sessionTemplateId
  // would collide on nulls exactly like the old one).
  const existing3 = await listNames();
  ...
  // Phase 4: OAuth purge — tokens (access AND refresh; refresh re-mints stored
...
async function dryRun(db, pre) {
  ...
  for (const [o, n] of RENAMES) console.log(`  renameCollection ${o} (${pre[o]} docs) → ${n}${pre[n] !== null ? ` [target exists with ${pre[n]} docs — ${pre[n] === 0 ? 'will drop empty leftover' : 'WILL ABORT'}]` : ''}`);
...
async function verify(db, current) {
  ...
  // The audit doc is the count baseline; without it the reconciliation is
  // meaningless — hard-fail rather than silently skipping.
  assert(!!audit, '_migration_audit preflight doc present (count baseline)');
  ...
  const zeroChecks = [
    ...
    ['plans', { $or: [
      { 'weeks.workouts': { $exists: true } },
      { 'schedule.workoutsPerWeek': { $exists: true } },
      { 'schedule.preferredWorkoutDays': { $exists: true } },
      { 'progress.completedWorkouts': { $exists: true } },
      { 'progress.totalWorkouts': { $exists: true } },
      { 'progress.skippedWorkouts': { $exists: true } },
    ] }],
  ...
  // Validator is installed by add-sessiontemplates-validator.js AFTER this
  // script — warn rather than fail so --verify can run in either order.
  const stInfo = await db.listCollections({ name: 'sessiontemplates' }).toArray();
  const hasValidator = !!(stInfo[0] && stInfo[0].options && stInfo[0].options.validator);
  console.log(`${hasValidator ? '✓' : '⚠'} sessiontemplates $jsonSchema validator ${hasValidator ? 'present' : 'NOT installed yet — run add-sessiontemplates-validator.js'}`);
...
main().catch((e) => { console.error(redact(e && e.message ? e.message : e)); process.exit(1); });
