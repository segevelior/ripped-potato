...
export function useStreamingChat() {
  ...
  const [statusMessage, setStatusMessage] = useState(''); // Track reflection status messages

  // AbortController ref for stopping streams
  const abortControllerRef = useRef(null);

  // Ref to track tools without stale closure issues
            ...
            if (eventData) {
              try {
                ...
                if (event.type === 'token') {
                  ...
                  // Store in ref for reliable access in tool_complete (avoids stale closure)
                  toolsMapRef.current.set(event.tool, event.description);
                  ...
                  // Get description from ref (avoids stale closure issue)
                  const toolDescription = toolsMapRef.current.get(event.tool);

                  // Update the tool marker in the message to show completion
                  if (toolDescription) {
                    accumulatedMessage = accumulatedMessage.replace(
                      `<tool-executing>${toolDescription}</tool-executing>`,
                      `<tool-complete>${toolDescription}</tool-complete>`
                    );
                    setStreamingMessage(accumulatedMessage);
                  }
                  ...
                  // Show reflection status indicator (e.g., "Reviewing plan...")
                  setStatusMessage(event.message);
                  console.log('[useStreamingChat] Status:', event.message);
                } else if (event.type === 'revision') {
                  // Replace accumulated content with revised version from reflection
                  accumulatedMessage = event.content;
                  setStreamingMessage(accumulatedMessage);
                  setStatusMessage(''); // Clear status after revision
                  console.log('[useStreamingChat] Response revised. Issues fixed:', event.issues_fixed);
                } else if (event.type === 'complete') {
                  // Capture the conversation ID for return
                  returnedConversationId = event.conversation_id;
                  setStatusMessage(''); // Clear any remaining status
